...
import { LeadInfoPropertiesIconMapping, LeadInfoSectionNamePropsMapping, LeadInfoSections } from "./constant";
  ...
  const getView = () => {
    const getPropView = (property) => {
        return (<div className="info-tile" key={property}>
            <div className="icon">
                <Icon icon={LeadInfoPropertiesIconMapping[property] || IconTypes.EMAIL_OL}/>
            </div>
            <div className="text">
                {property}
            </div>
    ...
    const getSections = () => {
        return (Object.values(LeadInfoSections).map((sectionName) => {
            ...
            {LeadInfoSectionNamePropsMapping[sectionName].map(
                (property) => getPropView(property)
            )}
            </div>
            </>)
        }))
